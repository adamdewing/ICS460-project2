...
public static int window_size;
	...
	public static String changeBit(String data) {
		...
		int bitErrorIndex = random.nextInt(16);
	...
	public static Packet makePacket(int nextSequenceNumber)
	{
		...
		binaryData = Payload.getBinaryData();
		String dataSum = Payload.sumData(binaryData);
		binaryChecksum = Payload.takeOnesComplement(dataSum);
		
		// Create a new packet with a new data, checksum and sequence number
		Packet pkt = new Packet(nextSequenceNumber, binaryData.getBytes(), binaryChecksum.getBytes(), window_size);
