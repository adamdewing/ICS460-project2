...
public class UDPDataSender implements DataSender {
    ...
    public void sendData(byte[] bytes, int packet_size, int timeout, String receiverIpAddress, int receiverPort, int windowSize, double errors) {
        ...
        try (DatagramSocket socket = new DatagramSocket(port)) {
            ...
            System.out.println("Need to send a total of " + sent_packet_list.size() + " packets.");
            ...
            while (true) {
                ...
                for (int i = nextFrameExpected; i < nextFrameExpected + windowSize; i++) {
                    if (i < isSent.length && (!isSent[i])) {
                        ...
                        NetworkSendLogger.DatagramCondition condition = NetworkSendLogger.DatagramCondition.SENT;
                        if (corruptDelayDropPacket(packet) != null) {
                            ...
                            if (packet.getCksum() == 1) {
                                ...
                                packet.setCksum((short) 0);  // Reset cksum otherwise will always be an error packet
                            }
                        } else {
...
//        }
    ...
    /**
     * Will randomly corrupt, delay, or drop a packet.
     *
     * @param packet
     * @return
     */
        ...
        if (Math.random() < errorRate) {
            ...
            if (error < .33) {
                ...
                try {
                    ...
                    System.out.println("Delaying send by " + delay + " seconds");
